...
Your results of Quizes can be avalable this area After a quiz. 
	...
	<form action="selecter.php" method="post">
		...
		while($retVal=mysql_fetch_array($result,MYSQL_ASSOC)){
			...
			//cell is quiz id	
				$qresult= mysql_query("SELECT stu_id FROM marks WHERE quiz_id='".$cell."'") or die("function error");
				while($qretVal=mysql_fetch_array($qresult,MYSQL_ASSOC)){
					foreach($qretVal as $qcell){
											$stu_id=$qcell;
													
													
												$mdowork ="SELECT marks FROM marks WHERE stu_id='".$stu_id."' AND quiz_id='".$cell."'";
												$mresult= mysql_query($mdowork) or die("function error");
												 ...
												 foreach($mretVal as $mcell){
													...
													echo '<input type="text" class="buttonbody"  readonly name="id_me8" value='.$stu_id.' onmouseover="bigImg(this)" onmouseout="normalImg(this)" />';
